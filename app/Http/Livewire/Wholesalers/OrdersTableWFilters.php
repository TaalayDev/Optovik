...
class OrdersTableWFilters extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.wholesalers.orders-table-w-filters', with(['orders' => $this->orders]));
    }
    ...
    public function filter($f, $v)
    {
        ...
        switch ($f)
        {
            case 'state':
                $this->state = $v;
                $this->orders = $this->state == 0 ? $woc->findBy('state', 1) : $woc->findBy('state', $v);
                break;
        }
    }
    ...
    public function delete($id)
    {
        ...
        $this->orders = $this->state == 0 ? $woc->findBy('state', 1) : $woc->findBy('state', $this->state);
    }
